...
public class IBXM {
	...
	/*
	 * Set the sampling rate of playback. This can be used with Module.c2Rate to
	 * adjust the tempo and pitch.
	 */
	...
	/*
	 * Mute or unmute the specified channel. If channel is negative, mute or unmute
	 * all channels.
	 */
	...
	/*
	 * Generate audio. The number of samples placed into outputBuf is returned. The
	 * output buffer length must be at least that returned by getMixBufferLength().
	 * A "sample" is a pair of 16-bit integer amplitudes, one for each of the stereo
	 * channels.
	 */
	...
	private int[] audioCache = null;
	private int audioCacheOffset = 0;
	private int audioCacheLimit = 0;

	public void getAudio(int[] outputBuf, int sampleCount) {
		int usedSampleCount = 0;
		while (usedSampleCount < sampleCount) {
			if (audioCache == null) {
				audioCache = new int[getMixBufferLength()];
			}
			if (audioCacheLimit == 0) {
				audioCacheLimit = getAudio(audioCache);
			}
			boolean flushed = true;
			for (int i = audioCacheOffset; i < audioCacheLimit; i++) {
				if (usedSampleCount >= sampleCount) {
					audioCacheOffset = i;
					flushed = false;
					break;
				}
				outputBuf[usedSampleCount] = audioCache[i];
				// System.out.println(audioCache[i]);
				usedSampleCount++;
			}
			if (flushed) {
				audioCache = null;
	...
	private void downsample(int[] buf, int count) {
		/*
		 * 2:1 downsampling with simple but effective anti-aliasing. Buf must contain
		 * count * 2 + 1 stereo samples.
		 */
		int outLen = count * 2;
