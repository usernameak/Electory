...
public class VoronoiTest extends JFrame {
	...
	public VoronoiTest() {
		...
		// final Color[] color_lookup = new Color[] { new Color(0xFF00FF00), new Color(0xFF0000FF) };
		...
		JPanel panel = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				for (int i = 0; i < 2048; i++) {
					for (int j = 0; j < 2048; j++) {
						Color color = colors[i][j] > 0.5f ? new Color(0f, colors[i][j], 0f) : new Color(0f, colors[i][j], colors[i][j]);
						g.setColor(color);
						g.fillRect(i, j, 1, 1);
					}
				}
