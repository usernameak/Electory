...
public class Chunk {
    ...
    public void setBlockWithMetadataAt(int x, int y, int z, Block block, Object meta, int flags) {
        ...
        if ((flags & World.FLAG_SKIP_LIGHT_UPDATE) == 0) {
            recalculateLightForBlock(x, y, z, (flags & World.FLAG_FAST_LIGHT_UPDATE) != 0);
        }
        if ((flags & World.FLAG_SKIP_RENDER_UPDATE) == 0) {
            scheduleRenderUpdateForBlockAndAdjacents(x, z);
        }
    ...
    private long recalculateLight(int lightLevelType, Queue<LightingQueueEntry> bfsSkyQueue) {
        ElectoryProfiler.INSTANCE.begin("skyLight");
        long stepCounter = 0;
        while (!bfsSkyQueue.isEmpty()) {
            LightingQueueEntry pos = bfsSkyQueue.poll();

            recalculateLightStep(lightLevelType, pos.x, pos.y, pos.z, pos.isDefinitelyIncreasing, bfsSkyQueue);
            stepCounter++;
        }
    ...
    private void recalculateLightStep(int lightLevelType, int x, int y, int z, boolean isDefinitelyIncreasing, Queue<LightingQueueEntry> bfsSkyQueue) {
        if (y >= 256 || y < 0) return;
        ...
        if (opacity != 15 && lightLevel != 15) {
            // find most affecting light source
            for (EnumSide side : EnumSide.VALID_DIRECTIONS_UPFIRST) {
                if (lightLevel == 15) break;
        ...
        if (lightLevel < oldLightLevel) { // light level decreased
            for (EnumSide side : EnumSide.VALID_DIRECTIONS_DOWNFIRST) {
                ...
                if (lightIntensityAdjacent < oldLightLevel
                    ...
                    bfsSkyQueue.add(new LightingQueueEntry(x + side.offsetX, y + side.offsetY, z + side.offsetZ, false));
                }
            }
            ...
            for (EnumSide side : EnumSide.VALID_DIRECTIONS_DOWNFIRST) {
                ...
                if (lightIntensityAdjacent < lightLevel - adjOpacity) {
                    ...
                    // add them to BFS queue
                    bfsSkyQueue.add(new LightingQueueEntry(x + side.offsetX, y + side.offsetY, z + side.offsetZ, true)); // kinda needs light update
                }
            }
        }
    }
    ...
    public void recalculateLight(int lightLevelType) {
        Queue<LightingQueueEntry> bfsSkyQueue = new LinkedList<>();
        for (int x = getChunkBlockCoordX(); x < getChunkBlockCoordX() + 16; x++) {
            for (int z = getChunkBlockCoordZ(); z < getChunkBlockCoordZ() + 16; z++) {
                bfsSkyQueue.add(new LightingQueueEntry(x, 255, z, true));
            }
        }
        long steps = recalculateLight(lightLevelType, bfsSkyQueue);
        TinyCraft.getInstance().logger.info("recalculateLight took " + steps + " steps");
    }
    ...
    public void recalculateLightForBlock(int lightLevelType, int x, int y, int z, boolean fast) {
        ...
        bfsSkyQueue.add(new LightingQueueEntry(wx, y, wz, false));
        recalculateLight(lightLevelType, bfsSkyQueue);
    }

    public void recalculateLightForBlock(int x, int y, int z, boolean fast) {
        recalculateLightForBlock(World.LIGHT_LEVEL_TYPE_SKY, x, y, z, false);
        recalculateLightForBlock(World.LIGHT_LEVEL_TYPE_BLOCK, x, y, z, fast);
    }
